...
def extract_signals_from_text(text):
    ...
    signals = []
    cleaned = text.replace('**', '').replace('⚠️', '').strip()
    
    patterns = [
        # Standard format: TOKEN - POSITION x LEVERAGE
        r'(?:\d+\.\s*)?([A-Za-z0-9]+(?:\s*[A-Za-z0-9]*)?(?:USDT)?)\s*[-–]\s*(Long|Short)(?:\s*[-–]\s*(?:Leverage\s*)?|\s*,\s*|\s+)x(\d+)',
        
        # Position first: POSITION TOKEN x LEVERAGE
        r'(?:\d+\.\s*)?(Long|Short)\s+([A-Za-z0-9]+(?:\s*[A-Za-z0-9]*)?(?:USDT)?)(?:\s*[-–]\s*(?:Leverage\s*)?)?x(\d+)',
        
        # Dash format: - POSITION TOKEN x LEVERAGE
        r'[-–]\s*(Long|Short)\s+([A-Za-z0-9]+(?:\s*[A-Za-z0-9]*)?(?:USDT)?)\s+x(\d+)',
        
        # Colon format: TOKEN: POSITION x LEVERAGE
        r'([A-Za-z0-9]+(?:\s*[A-Za-z0-9]*)?(?:USDT)?):?\s*(Long|Short)(?:\s*[-–]\s*|\s*,\s*|\s+)x(\d+)',
        
        # Parentheses format: TOKEN (POSITION) x LEVERAGE
        r'([A-Za-z0-9]+(?:\s*[A-Za-z0-9]*)?(?:USDT)?)\s*\((Long|Short)\)\s*x(\d+)',
